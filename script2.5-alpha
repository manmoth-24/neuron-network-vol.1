...
var network = new NeuronNetwork(5, [2,3,5,3,2])
...
function RunAI(nw, input, teacher, rate){
    ...
    var activationList = ["sigmoid", "sigmoid", "sigmoid", "sigmoid"]
...
function OnlyLearning(nw, input, teacher, rate){
    ...
    var activationList = ["sigmoid", "sigmoid", "sigmoid", "identify"]
    var result = nw.Play(inputVector, activationList)
...
setInterval(()=>{
    var rate = 0.1 + getRandomArbitrary(-1, 1) * 0.0001
    for (var i = 0; i < 10; i++){
            ...
            OnlyLearning(network, input, desired, rate)
    ...
    var input = [
            ...
            RunAI(network, input, desired, rate)
